...
def train_seg(net, train_data=None, train_labels_c=None, train_labels_o=None,
    ...
    # =================================================================
    # PARAMETER FREEZE LOGIC (Updated for FPN Parasites)
    # =================================================================
    ...
    if is_frozen:
        if unfreeze_epoch >= n_epochs:
            ...
            train_logger.info(f"\n>>> [MODELS] Phase 1: Freezing Swin Backbone. Will unfreeze at Epoch {unfreeze_epoch} ({unfreeze_backbone}%).")
    else:
    ...
    for name, param in net.named_parameters():
        if any(x in name for x in ['cell_decoder', 'org_decoder']):
            ...
            param.requires_grad = not is_frozen # Backbone follows freeze schedule
        else:
    ...
    for iepoch in range(n_epochs):
        ...
        # =================================================================
        # AUTO-UNFREEZE TRIGGER (Targeting Swin Backbone)
        # =================================================================
        if is_frozen and iepoch >= unfreeze_epoch and unfreeze_epoch < n_epochs:
            ...
            for name, param in net.named_parameters(): 
                if not param.requires_grad and 'backbone' in name:
                    param.requires_grad = True
