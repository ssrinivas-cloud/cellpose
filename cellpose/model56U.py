...
Modified to include Omnipose Scale-Invariant, Distance-Field and Divergence-Rescaled Pipeline.
"""
...
class CellposeModel():
    ...
    def eval(self, x, batch_size=8, resample=True, channels=None, channel_axis=None,
        ...
        # =================================================================
        # >>> OMNIPOSE UPDATE: SCALE MANAGEMENT (ZERO RESCALING) <<<
        # =================================================================
        image_scaling = {'cells': 1.0, 'organelles': 1.0}
        self.use_omni_solver = False
        
        if diameter is not None:
            if isinstance(diameter, (list, tuple, np.ndarray)) and len(diameter) >= 2:
                image_scaling['cells'] = 30. / diameter[0] if diameter[0] > 0 else 1.0
                image_scaling['organelles'] = 30. / diameter[1] if diameter[1] > 0 else 1.0
                if diameter[0] == 0: self.use_omni_solver = True
            elif isinstance(diameter, dict):
                d_cell = diameter.get('cells', 0)
                d_org = diameter.get('organelles', 0)
                image_scaling['cells'] = 30. / d_cell if d_cell > 0 else 1.0
                image_scaling['organelles'] = 30. / d_org if d_org > 0 else 1.0
                if d_cell == 0: self.use_omni_solver = True
            elif isinstance(diameter, (int, float)):
                if diameter > 0:
                    image_scaling['cells'] = 30. / diameter
                    image_scaling['organelles'] = 30. / diameter
                else:
                    image_scaling['cells'] = 1.0
                    image_scaling['organelles'] = 1.0
                    self.use_omni_solver = True
    ...
    # =================================================================
    # >>> OMNIPOSE ENGINE: DISTANCE FIELD & DIVERGENCE INTEGRATION <<<
    # =================================================================
    def _compute_omni_tracking_2d(self, dP, cellprob, cellprob_threshold=0.0, niter=200):
        """
        Omnipose Vector Tracking Core:
        Uses Signed Distance Field flows rescaled by Divergence maps
        to aggregate pixels smoothly onto the structural skeleton.
        """
        dy, dx = dP[0], dP[1]
        
        # 1. Calculate Spatial Divergence of predicted flow gradients
        grad_yy, grad_yx = np.gradient(dy)
        grad_xy, grad_xx = np.gradient(dx)
        divergence = grad_yy + grad_xx
        
        # Smooth divergence to handle micro-textures/noise from Brightfield images
        div_smooth = gaussian_filter(divergence, sigma=1.0)
        
        # 2. Rescale predicted flows based on structural divergence profiles
        # Attenuates velocities at distance peak sinks (negative divergence)
        # Accentuates separation boundaries (positive divergence)
        v_mod = np.clip(1.0 + div_smooth, 0.1, 2.0)
        dy_mod = dy * v_mod
        dx_mod = dx * v_mod
        ...
        # Construct mask tracking vectors based on background rejection logit thresholds
        mask_indices = cellprob > cellprob_threshold
        if not np.any(mask_indices):
            return np.zeros((H, W), dtype=np.int32)
            
        y_pts = grid_y[mask_indices].astype(np.float32)
        x_pts = grid_x[mask_indices].astype(np.float32)
        
        # 3. Euler Integration mapping paths across the cell anatomy topology
        for _ in range(niter):
            iy = np.clip(np.round(y_pts).astype(int), 0, H - 1)
            ix = np.clip(np.round(x_pts).astype(int), 0, W - 1)
            
            y_pts += dy_mod[iy, ix]
            x_pts += dx_mod[iy, ix]
            
            y_pts = np.clip(y_pts, 0, H - 1)
            x_pts = np.clip(x_pts, 0, W - 1)
            
        iy_final = np.clip(np.round(y_pts).astype(int), 0, H - 1)
        ix_final = np.clip(np.round(x_pts).astype(int), 0, W - 1)
        
        # 4. Connected components analysis along the unified skeleton
        dest_map = np.zeros((H, W), dtype=int)
        dest_map[iy_final, ix_final] = 1
        labeled_peaks, _ = label(dest_map)
        
        final_masks = np.zeros((H, W), dtype=np.int32)
        final_masks[mask_indices] = labeled_peaks[iy_final, ix_final]
        return final_masks


    def _compute_masks(self, shape, dP, cellprob, flow_threshold=0.4, cellprob_threshold=0.0,
                       min_size=15, max_size_fraction=0.4, niter=None,
                       do_3D=False, stitch_threshold=0.0, head_type='cells'):
        ...
        # Trigger Omnipose Distance Field & Divergence mapping logic for scale-invariant cells
        if self.use_omni_solver and not do_3D and head_type == 'cells':
            nimg = shape[0]
            for i in range(nimg):
                outputs = self._compute_omni_tracking_2d(dP[:, i], cellprob[i], 
                                                         cellprob_threshold=cellprob_threshold, 
                                                         niter=niter if niter is not None else 200)
                if i == 0:
                    masks = np.zeros((nimg, shape[1], shape[2]), dtype=np.int32)
                masks[i] = outputs
        ...
        # Fallback to standard tracking pathway if forced scale sizes are given or for organelles
        if do_3D:
        ...
        if changed_device_from is not None:
            self.net.device = torch.device(changed_device_from)
        return masks
