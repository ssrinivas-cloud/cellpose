                ...
                for row, head in enumerate(heads_to_process):
                    ...
                    if ground_truth is not None:
                        ...
                        if np.any(true_mask > 0):
                            # Vectorized thick boundaries (Lime Green for GT)
                            bounds_gt = find_boundaries(true_mask, mode='inner')
                            if thickness > 1:
                                bounds_gt = binary_dilation(bounds_gt, structure=struct_element)
                            
                            overlay_gt = np.zeros((*img_show.shape, 4))
                            overlay_gt[bounds_gt] = [0.0, 1.0, 0.0, alpha_level] # [R, G, B, A]
                    ...
                    # ---------------------------------------------------------
                    # 2. Predicted Mask Subplot (Right)
                    # ---------------------------------------------------------
                    axes[row][1].imshow(img_show, cmap='gray')
                    if np.any(pred_mask > 0):
                        # Vectorized thick boundaries (Bright Red for Prediction)
                        bounds_pred = find_boundaries(pred_mask, mode='inner')
                        if thickness > 1:
                            bounds_pred = binary_dilation(bounds_pred, structure=struct_element)
                        
                        overlay_pred = np.zeros((*img_show.shape, 4))
                        overlay_pred[bounds_pred] = [1.0, 0.0, 0.0, alpha_level] # [R, G, B, A]
